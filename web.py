...
def init_pg_schema():
    ...
    try:
        ...
        for alter in [
            ...
            "ALTER TABLE stock ADD COLUMN IF NOT EXISTS motif TEXT DEFAULT ''",
...
def _dashboard_work():
    ...
    try:
        if etid is not None:
            row = db_fetchone(conn, "SELECT (SELECT COUNT(*) FROM produits WHERE tenant_id=%s) as nb_prod, (SELECT COUNT(*) FROM ventes WHERE tenant_id=%s AND date=%s) as nb_ventes, (SELECT COALESCE(SUM(total_usd),0) FROM ventes WHERE tenant_id=%s AND date=%s) as ca, (SELECT COUNT(DISTINCT client_nom) FROM ventes WHERE tenant_id=%s AND date=%s AND client_nom!='') as nb_clients, (SELECT COUNT(*) FROM produits WHERE tenant_id=%s AND stock<=5) as low_stock, (SELECT COUNT(*) FROM dettes WHERE tenant_id=%s AND est_paye=0) as nb_dettes, (SELECT COALESCE(SUM(montant_usd),0) FROM dettes WHERE tenant_id=%s AND est_paye=0) as total_dettes" if IS_PG else
                              "SELECT (SELECT COUNT(*) FROM produits WHERE tenant_id=?) as nb_prod, (SELECT COUNT(*) FROM ventes WHERE tenant_id=? AND date=?) as nb_ventes, (SELECT COALESCE(SUM(total_usd),0) FROM ventes WHERE tenant_id=? AND date=?) as ca, (SELECT COUNT(DISTINCT client_nom) FROM ventes WHERE tenant_id=? AND date=? AND client_nom!='') as nb_clients, (SELECT COUNT(*) FROM produits WHERE tenant_id=? AND stock<=5) as low_stock, (SELECT COUNT(*) FROM dettes WHERE tenant_id=? AND est_paye=0) as nb_dettes, (SELECT COALESCE(SUM(montant_usd),0) FROM dettes WHERE tenant_id=? AND est_paye=0) as total_dettes",
                              (etid, etid, today, etid, today, etid, today, etid, etid, etid))
        else:
            row = db_fetchone(conn, "SELECT (SELECT COUNT(*) FROM produits) as nb_prod, (SELECT COUNT(*) FROM ventes WHERE date=%s) as nb_ventes, (SELECT COALESCE(SUM(total_usd),0) FROM ventes WHERE date=%s) as ca, (SELECT COUNT(DISTINCT client_nom) FROM ventes WHERE date=%s AND client_nom!='') as nb_clients, (SELECT COUNT(*) FROM produits WHERE stock<=5) as low_stock, (SELECT COUNT(*) FROM dettes WHERE est_paye=0) as nb_dettes, (SELECT COALESCE(SUM(montant_usd),0) FROM dettes WHERE est_paye=0) as total_dettes" if IS_PG else
                              "SELECT (SELECT COUNT(*) FROM produits) as nb_prod, (SELECT COUNT(*) FROM ventes WHERE date=?) as nb_ventes, (SELECT COALESCE(SUM(total_usd),0) FROM ventes WHERE date=?) as ca, (SELECT COUNT(DISTINCT client_nom) FROM ventes WHERE date=? AND client_nom!='') as nb_clients, (SELECT COUNT(*) FROM produits WHERE stock<=5) as low_stock, (SELECT COUNT(*) FROM dettes WHERE est_paye=0) as nb_dettes, (SELECT COALESCE(SUM(montant_usd),0) FROM dettes WHERE est_paye=0) as total_dettes",
                              (today, today, today))
        if row:
...
def stock():
    ...
    for t in tenants:
        ...
        historique = db_fetchall(conn, """SELECT s.date_mouvement, s.mouvement, p.nom, s.code_produit, s.couleur, s.quantite, u.login, s.motif
                ...
                WHERE s.tenant_id=%s ORDER BY s.date_mouvement DESC LIMIT 50""" if IS_PG else """SELECT s.date_mouvement, s.mouvement, p.nom, s.code_produit, s.couleur, s.quantite, u.login, s.motif
                FROM stock s JOIN produits p ON s.product_id=p.id JOIN utilisateurs u ON s.user_id=u.id
...
def ventes():
    ...
    if request.method == "POST":
        ...
        contenu = f"{recu_num}-{int(float(total_usd))}-{int(float(total_cdf))}-{client_nom}-{client_tel}-{now_date}-{now_heure}-{tid_sale}"
...
def ventes_validate():
    ...
    # Signature SHA256
    cle_secrete = app.secret_key
    contenu = f"{recu_num}-{int(float(total_usd_all))}-{int(float(total_cdf_all))}-{client_nom}-{client_tel}-{now_date}-{now_heure}-{tid_sale}"
    signature = hashlib.sha256((contenu + cle_secrete).encode()).hexdigest()[:12]
...
def recu_edit(rid):
    ...
    if request.method == "POST":
        ...
        cle_secrete = app.secret_key
        contenu = f"{recu['numero']}-{int(float(total_usd))}-{int(float(total_cdf))}-{client_nom}-{client_tel}-{recu['date']}-{recu['heure']}-{recu['tenant_id']}"
        signature = hashlib.sha256((contenu + cle_secrete).encode()).hexdigest()[:12]
...
def verifier_recu(numero=None):
    ...
    cn = r['client_nom'] or ''
    ct = r['client_tel'] or ''
    vu = str(int(float(r['total_usd'])))
    vc = str(int(float(r['total_cdf'])))
    fu = str(r['total_usd'])
    fc = str(r['total_cdf'])
    valide = False
    for usd_s in [vu, fu]:
        for cdf_s in [vc, fc]:
            contenu = f"{r['numero']}-{usd_s}-{cdf_s}-{cn}-{ct}-{r['date']}-{r['heure']}-{r['tenant_id']}"
            if hashlib.sha256((contenu + cle_secrete).encode()).hexdigest()[:12] == stored:
                valide = True
                break
        if valide:
            break
